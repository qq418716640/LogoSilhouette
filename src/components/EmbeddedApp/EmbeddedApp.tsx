...
/**
 * Threshold 和 Invert colors 控制组件
 * 从高级设置中提取出来，更容易被用户发现
 */
function ThresholdAndInvert() {
    ...
    <div className="space-y-3 md:space-y-4 p-3 md:p-4 bg-gray-50 rounded-lg">
      ...
      <div className="space-y-1.5 md:space-y-2">
        <div className="flex justify-between">
          <label className="text-xs md:text-sm font-medium text-gray-700">Threshold</label>
          <span className="text-xs md:text-sm text-gray-500">{params.threshold}</span>
        </div>
        <input
          type="range"
          min={50}
        ...
        <p className="text-xs text-gray-400">
          Pixels darker than the threshold will be converted to black
        </p>
      </div>
      ...
      <div className="space-y-1.5 md:space-y-2">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-1.5 md:gap-2">
            ...
            <label className="text-xs md:text-sm font-medium text-gray-700">Invert colors</label>
          </div>
          <button
          ...
          >
            <span
              className={`
                absolute top-0.5 left-0.5 w-4 h-4 md:w-5 md:h-5 bg-white rounded-full shadow
                transition-transform ${params.invert ? 'translate-x-[1.125rem] md:translate-x-5' : ''}
              `}
            />
          </button>
        </div>
        <p className="text-xs text-gray-400">
          It is recommended to turn it on when the subject is light-colored against a dark background
        </p>
      </div>
    </div>
  )
    ...
    <div className="max-w-4xl mx-auto px-3 md:px-4">
      <div className="bg-white rounded-2xl shadow-sm border border-gray-200 overflow-hidden">
        <div className="grid md:grid-cols-2 gap-0">
          {/* 左侧：控制面板 */}
          <div className="p-4 md:p-6 space-y-4 md:space-y-6 border-b md:border-b-0 md:border-r border-gray-200">
            {/* 上传区域 */}
            {!sourceImage ? (
              <ImageUploader />
            ) : (
              <div className="space-y-3 md:space-y-4">
                ...
                <div className="flex items-center justify-between p-2.5 md:p-3 bg-gray-50 rounded-lg">
                  <div className="flex items-center gap-2 md:gap-3 min-w-0">
                    ...
                    <div className="min-w-0">
                      <p className="text-xs md:text-sm font-medium text-gray-700 truncate max-w-[120px] md:max-w-[150px]">
                  ...
                  <button
                    ...
                    className="p-1.5 md:p-2 text-gray-400 hover:text-gray-600 hover:bg-gray-100 rounded-lg transition-colors flex-shrink-0"
                  >
                ...
                {/* Threshold 和 Invert colors - 常用选项，放在外面 */}
                <ThresholdAndInvert />
